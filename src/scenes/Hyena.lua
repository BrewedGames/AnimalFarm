...
local hyena, hyenaSprite, hyenaCollider = initEntity("hyena", "./static/hyena.png", HyenaSpriteWidth, HyenaSpriteHeight, Vec3(500, 500, 0), 60, "hyena")
...
local function throwKnife1(delta_time)
    if isKnifeActive1 then
        ...
        if playerCollision(playerCollider, knifeCollider1) then
            ...
            hyenaSprite:loadSprite(hyenaNormalSpritePath, HyenaSpriteWidth, HyenaSpriteHeight, hyenaSprite:getPos())
        end
    ...
    elseif knifeTimer1 <= 0 then
        ...
        if distance > chaseDistance then
            ...
            hyenaSprite:loadSprite(hyenaAttackSpritePath, HyenaSpriteWidth, HyenaSpriteHeight, hyenaPos)
        end
    else
        knifeTimer1 = knifeTimer1 - delta_time
    end
...
local function updateRect(delta_time)
    if rectActive then
        ...
        if rectAngle >= 360 then
            rectAngle = rectAngle - 360
        end

        local hyenaPos = hyenaSprite:getPos()
        local rectLength = 250
        local freeEndX, freeEndY = rotatePointAroundOrigin(
            hyenaPos.x + rectLength, hyenaPos.y,
            hyenaPos.x, hyenaPos.y,
            rectAngle
        )
