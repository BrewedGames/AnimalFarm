...
local background, backgroundSprite = initEntity("Background", "./static/rhena_map.jpg", 1280, 720, Vec3(640, 360, 0))

--init rhino entity w image and collider
local rhino, rhinoSprite, rhinoCollider = initEntity("rhino", "./static/rhena.png", RhinoSpriteWidth, RhinoSpriteHeight, Vec3(500, 500, 0), 60, "rhino")

--init sausage entity with image collider and where it is off screen
local sausage, sausageSprite, sausageCollider = initEntity("sausage", "./static/sausage.png", 20, 10, Vec3(-1000, -1000, 0), 10, "sausage")
...
local rightEnt, rightCol = initRectCol("right", Vec3(1170, 90, 0), 100, 320, "right") --right desk
local rightBEnt, rightBCol = initRectCol("rightbottom", Vec3(1050, 20, 0), 320, 70, "rightbottom") --right bottom L desk
...
local function throwSausage(delta_time)
    if isSausageActive then --if the sausage being thrown
        ...
        if playerCollision(playerCollider, sausageCollider) then
            ...
            --reset rhino sprite
            rhinoSprite:loadSprite(rhinoNormalSpritePath, RhinoSpriteWidth, RhinoSpriteHeight, rhinoSprite:getPos())
        end
    ...
    elseif sausageTimer <= 0 then --if sausage not active
        ...
        if distance > chaseDistance then --if distance is further than the other ranges
            ...
            rhinoSprite:loadSprite(rhinoAttackSpritePath, RhinoSpriteWidth, RhinoSpriteHeight, rhinoPos) 
        end
    else
